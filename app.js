...
const FALLBACK_HOME = {
  ...
  heroImage: 'assets/hero-inicio-optimized.webp',
...
function renderFavoritesDrawer(){
  ...
  target.innerHTML = items.map((product) => `
    <article class="favorite-item">
      <a class="favorite-thumb" href="${getProductUrl(product)}"><img ${optimizedImageAttrs(product.image, product.name)}></a>
...
function renderCartDrawer(){
  ...
  target.innerHTML = state.cart.map((item) => `
    <article class="cart-item">
      <div class="cart-thumb"><img ${optimizedImageAttrs(item.image, item.name)}></div>
      <div class="cart-meta">
...
function renderSearchResults(products, query){
  ...
  target.innerHTML = results.map((product) => `
    <article class="search-item">
      <a class="search-thumb" href="${getProductUrl(product)}"><img ${optimizedImageAttrs(product.image, product.name)}></a>
...
function getOptimizedAssetPath(src){
  const value = String(src || '').trim();
  if(!value || /^(https?:|data:|blob:)/i.test(value)) return value;
  if(!value.startsWith('assets/') || value.startsWith('assets/optimized/')) return value;
  const clean = value.split('?')[0].split('#')[0];
  if(!/\.(png|jpe?g|webp)$/i.test(clean)) return value;
  const fileName = clean.split('/').pop().replace(/\.[^.]+$/, '.webp');
  return `assets/optimized/${fileName}`;
}

function optimizedImageAttrs(src, alt = '', extra = ''){
  const original = String(src || '').trim();
  const optimized = getOptimizedAssetPath(original);
  const fallback = original && optimized !== original ? ` data-fallback-src="${escapeHtml(original)}" onerror="this.onerror=null;this.src=this.dataset.fallbackSrc;"` : '';
  return `src="${escapeHtml(optimized || original)}" alt="${escapeHtml(alt)}"${extra ? ` ${extra}` : ''}${fallback}`;
}
...
function createHoverMedia(mediaClassName, product, productUrl){
  ...
  const hoverMarkup = hover
    ? `<img class="product-media-secondary" ${optimizedImageAttrs(hover, '', 'loading="lazy" aria-hidden="true"')}>`
    : '';

  return `
    <a class="${mediaClassName}${hover ? ' has-hover-image' : ''}" href="${productUrl}">
      <img class="product-media-primary" ${optimizedImageAttrs(product.image, product.name, 'loading="lazy" decoding="async"')}>
      ${hoverMarkup}
    </a>
  `;
}
...
async function initHomePage(){
  ...
  if(heroSection){
    ...
    const heroImage = home.heroImage || FALLBACK_HOME.heroImage;
    if(heroImage){
...
if(stickyImage){
  ...
  stickyImage.src = getOptimizedAssetPath(product.image);
  if(getOptimizedAssetPath(product.image) !== product.image) stickyImage.dataset.fallbackSrc = product.image;
  stickyImage.onerror = () => { stickyImage.onerror = null; stickyImage.src = stickyImage.dataset.fallbackSrc || stickyImage.src; };
  ...
  mainImage.src = getOptimizedAssetPath(gallery[currentIndex]);
  if(getOptimizedAssetPath(gallery[currentIndex]) !== gallery[currentIndex]) mainImage.dataset.fallbackSrc = gallery[currentIndex];
  mainImage.onerror = () => { mainImage.onerror = null; mainImage.src = mainImage.dataset.fallbackSrc || mainImage.src; };
  ...
  thumbs.innerHTML = gallery.map((image, index) => `
    <button class="product-thumb ${index === 0 ? 'active' : ''}" type="button" data-thumb-index="${index}">
      <img ${optimizedImageAttrs(image, `${product.name} ${index + 1}`)} loading="lazy">
    </button>
  `).join('');
