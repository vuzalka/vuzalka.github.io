...
const FALLBACK_HOME = {
  ...
  heroImage: 'assets/hero-inicio.webp',
...
function getHoverImage(product){
  ...
  return gallery.find((image) => image && image !== product.image) || '';
}

function buildHoverMedia(product, mediaClass, productUrl, { loading = 'eager' } = {}){
  const hoverImage = getHoverImage(product);
  const mediaClasses = hoverImage ? `${mediaClass} has-hover-image` : mediaClass;
  const loadingAttr = loading ? ` loading="${loading}"` : '';

  return `
    <a class="${mediaClasses}" href="${productUrl}">
      <img class="media-image media-image--primary" src="${escapeHtml(product.image)}" alt="${escapeHtml(product.name)}"${loadingAttr}>
      ${hoverImage ? `<img class="media-image media-image--hover" src="${escapeHtml(hoverImage)}" alt="${escapeHtml(product.name)} vista alterna"${loadingAttr}>` : ''}
    </a>
  `;
}
...
function createCollectionCard(product){
  ...
  return `
    <article class="collection-card">
      ${buildHoverMedia(product, 'collection-media', productUrl, { loading: 'lazy' })}
...
function createCatalogCard(product){
  ...
  return `
    <article class="catalog-card" data-card-link="${productUrl}" role="link" tabindex="0" aria-label="Ver ${escapeHtml(product.name)}">
      ...
      ${buildHoverMedia(product, 'catalog-media', productUrl)}
...
function createRecommendCard(product){
  ...
  return `
    <article class="rec-card">
      ...
      ${buildHoverMedia(product, 'rec-card-media', productUrl)}
...
function renderHomeCollection(products, filter = 'all'){
  ...
  if(count){
    count.textContent = `${selection.length} ${selection.length === 1 ? 'perfume visible' : 'perfumes visibles'}`;
  }
...
async function initHomePage(){
  ...
  if(heroSection){
    ...
    if(heroTitle) heroTitle.textContent = home.heroTitle || FALLBACK_HOME.heroTitle;
