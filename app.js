...
const OPTIMIZED_IMAGE_STEMS = new Set(["9pm-sin-fondo", "acqua-di-gio", "acqua-di-gio-edt-sin-fondo", "acqua-di-gio-producto", "afnan-9pm", "afnan-9pm-producto", "armaf-club-de-nuit-intense", "club-de-nuit-man-sin-fondo", "club-de-nuit-producto", "eros-flame", "hero-inicio", "hugo-boss-bottled", "hugo-boss-bottled-producto", "hugo-boss-bottled-sin-fondo", "hugo-boss-bottled-vi", "khamrah-lattafa-producto", "khamrah-lattafa-qahwa-producto", "lattafa-khamrah", "lattafa-khamrah-qahwa", "lattafa-khamrah-qahwa-sin-fondo", "lattafa-khamrah-sin-fondo", "odyssey-mandarin-sky", "odyssey-mandarin-sky-sin-fondo", "odyssey-mandarinsky-producto", "one-million", "one-million-edt-sin-fondo", "versace-eros-flame", "versace-eros-flame-producto", "versace-eros-flame-sin-fondo"]);

function getOptimizedImageSource(src){
  if(!src) return src;
  ...
  if(OPTIMIZED_IMAGE_STEMS.has(stem)){
    if(stem === 'hero-inicio') return 'assets/optimized/hero-inicio-1600.webp';
    return `assets/optimized/${stem}.webp`;
  }
  return value;
}

function prepareProductImages(product){
  const gallery = Array.isArray(product.gallery) ? product.gallery.map(getOptimizedImageSource).filter(Boolean) : [];
  const image = getOptimizedImageSource(product.image || gallery[0] || '');
  return { ...product, image, gallery: gallery.length ? gallery : (image ? [image] : []) };
}
...
const FALLBACK_HOME = {
  ...
  heroImage: 'assets/optimized/hero-inicio-1600.webp',
  heroKicker: 'Colección destacada',
  primaryButtonText: 'Explorar selección',
  primaryButtonLink: 'perfumes.html',
  secondaryButtonText: 'Ver catálogo completo',
  secondaryButtonLink: 'perfumes.html',
...
function normalizeGallery(docData){
  const gallery = [];
  const main = getOptimizedImageSource(docData.imagen || docData.image || '');
  if(main) gallery.push(main);
  const extras = docData.imagenes || docData.images || [];
  parseMultiValue(extras).forEach((item) => {
    const optimizedItem = getOptimizedImageSource(item);
    if(optimizedItem && !gallery.includes(optimizedItem)) gallery.push(optimizedItem);
  });
  return gallery;
}
...
function normalizeProduct(raw, id){
  ...
  const normalized = {
    ...
    image: getOptimizedImageSource(raw.imagen || raw.image || gallery[0] || ''),
    gallery: gallery.length ? gallery : (raw.imagen || raw.image ? [getOptimizedImageSource(raw.imagen || raw.image)] : []),
...
async function getProducts(){
  ...
  state.productsPromise = (async () => {
    try{
      ...
      if(products.length){
        state.productsCache = products.map(prepareProductImages);
    ...
    state.productsCache = FALLBACK_PRODUCTS.map(prepareProductImages);
    return state.productsCache;
  })();
...
function renderFavoritesDrawer(){
  ...
  target.innerHTML = items.map((product) => `
    <article class="favorite-item">
      <a class="favorite-thumb" href="${getProductUrl(product)}"><img src="${escapeHtml(product.image)}" alt="${escapeHtml(product.name)}" loading="lazy" decoding="async"></a>
...
function renderCartDrawer(){
  ...
  target.innerHTML = state.cart.map((item) => `
    <article class="cart-item">
      <div class="cart-thumb"><img src="${escapeHtml(item.image)}" alt="${escapeHtml(item.name)}" loading="lazy" decoding="async"></div>
      <div class="cart-meta">
...
function renderSearchResults(products, query){
  ...
  target.innerHTML = results.map((product) => `
    <article class="search-item">
      <a class="search-thumb" href="${getProductUrl(product)}"><img src="${escapeHtml(product.image)}" alt="${escapeHtml(product.name)}" loading="lazy" decoding="async"></a>
...
function createHoverMedia(mediaClassName, product, productUrl){
  const primary = escapeHtml(product.image || '');
  const hover = getHoverImage(product);
  const hoverMarkup = hover
    ? `<img class="product-media-secondary" src="${escapeHtml(hover)}" alt="" loading="lazy" aria-hidden="true">`
    : '';

  return `
    <a class="${mediaClassName}${hover ? ' has-hover-image' : ''}" href="${productUrl}">
      <img class="product-media-primary" src="${primary}" alt="${escapeHtml(product.name)}" loading="lazy" decoding="async">
      ${hoverMarkup}
    </a>
  `;
}
...
function createCatalogCard(product){
  const productUrl = getProductUrl(product);
  return `
    <article class="catalog-card" data-card-link="${productUrl}" role="link" tabindex="0" aria-label="Ver ${escapeHtml(product.name)}">
      ...
      ${createHoverMedia('catalog-media', product, productUrl)}
...
async function initHomePage(){
  ...
  if(heroSection){
    ...
    const heroImage = getOptimizedImageSource(home.heroImage || FALLBACK_HOME.heroImage);
    if(heroImage){
