...
function createCatalogCard(product){
  ...
  const hover = getHoverImage(product);
  return `
    <article class="catalog-card-premium" data-card-link="${productUrl}" role="link" tabindex="0" aria-label="Ver ${escapeHtml(product.name)}">
      <div class="catalog-card-media-wrap${hover ? ' has-hover-image' : ''}">
        ...
        <div class="catalog-card-overlay">
          <a class="catalog-card-cta" href="${productUrl}">Ver perfume</a>
        </div>
        <div class="catalog-card-badges">
          ${buildStockPill(product.stock)}
        </div>
        <div class="catalog-card-fav">
          ${createFavoriteButton(product.id)}
        </div>
      </div>
      <div class="catalog-card-body">
        <div class="catalog-card-brand">${escapeHtml(product.brand)}</div>
        <h3 class="catalog-card-name"><a href="${productUrl}">${escapeHtml(product.name)}</a></h3>
        <div class="catalog-card-price-row">
          ${product.compareAtPrice ? `<span class="catalog-card-was">${formatMoney(product.compareAtPrice)}</span>` : ''}
...
async function initHomePage(){
  ...
  if(heroSection){
    ...
    applyHeroImage(home.heroImage || FALLBACK_HOME.heroImage);
...
async function initCatalogPage(){
  ...
  function render(){
    ...
    resultCount.textContent = `${filtered.length} ${filtered.length === 1 ? 'perfume' : 'perfumes'}`;

    if(!filtered.length){
      grid.innerHTML = '<div class="catalog-empty-premium"><h3>Sin resultados</h3><p>No encontramos perfumes con esos filtros.</p></div>';
      return;
    }
